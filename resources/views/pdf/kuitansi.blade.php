...
<head>
    <meta charset="UTF-8">
    <title>Kuitansi / SPJ Pembayaran</title>
    <style>
        ...
        .table-data { w\idth: 100%; border-collapse: collapse; margin-bottom: 30px; }
        .table-data td { padding: 8px; vertical-align: top; }
        .table-data td:first-child { w\idth: 25%; font-weight: bold; }
        .table-data td:nth-child(2) { w\idth: 5%; }
...
<body>
    ...
    <table class="table-data">
        ...
        <tr>
            ...
            <td>:</td>
            <td>Kuasa Pengguna Anggaran BPJN</td>
        </tr>
        <tr>
            <td>Penerima / Toko</td>
            <td>:</td>
            <td>{{ $archive->penerima }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>:</td>
            <td>{{ $archive->tahun_anggaran ?? date('Y') }}</td>
        </tr>
    </table>
    ...
    @if(count($lampiranFiles) > 0)
        @foreach($lampiranFiles as $index => $path)
            ...
            @if($imgBase64)
                ...
                <div class="lampiran-title">LAMPIRAN BUKTI TRANSAKSI #{{ $index + 1 }}</div>
